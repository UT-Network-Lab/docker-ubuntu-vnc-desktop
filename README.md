...
```shell
docker run -p 6080:80 -v /dev/shm:/dev/shm utnetlab/web-desktop
```
...
Choose your favorite Ubuntu version with [tags](https://hub.docker.com/r/utnetlab/web-desktop/tags/)
...
```shell
docker run -p 6080:80 -p 5900:5900 -v /dev/shm:/dev/shm utnetlab/web-desktop
```

Now, open the vnc viewer and connect to port 5900. If you would like to protect vnc service by password, set environment variable `VNC_PASSWORD`, for example

```shell
docker run -p 6080:80 -p 5900:5900 -e VNC_PASSWORD=mypassword -v /dev/shm:/dev/shm utnetlab/web-desktop
```
...
```shell
docker run -p 6080:80 -e HTTP_PASSWORD=mypassword -v /dev/shm:/dev/shm utnetlab/web-desktop
```
...
```shell
docker run -p 6081:443 -e SSL_PORT=443 -v ${PWD}/ssl:/etc/nginx/ssl -v /dev/shm:/dev/shm utnetlab/web-desktop
```
...
```shell
docker run -p 6080:80 -e RESOLUTION=1920x1080 -v /dev/shm:/dev/shm utnetlab/web-desktop
```
...
```shell
docker run -p 6080:80 -e USER=doro -e PASSWORD=password -v /dev/shm:/dev/shm utnetlab/web-desktop
```
...
```shell
docker run -p 6080:80 -e RELATIVE_URL_ROOT=some-prefix utnetlab/web-desktop
```
...
```shell
docker run -it --rm -p 6080:80 --device /dev/snd -e ALSADEV=hw:2,0 utnetlab/web-desktop
```
